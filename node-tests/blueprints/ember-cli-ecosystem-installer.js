...
describe('Acceptance: ember generate ember-cli-ecosystem-installer', function () {
  ...
  var prompt, pkgsToInstall, addonsToInstall, pkgsToUninstall
  beforeEach(function () {
    // Mock the UI prompt to avoid prompting during tests
    prompt = td.function()
    td.replace(MockUI.prototype, 'prompt', prompt)

    // Mock the behavior on packages install
    var pkgsInstallTaskRun = td.function()
    td.when(pkgsInstallTaskRun(td.matchers.anything())).thenDo(function (task) { pkgsToInstall = task.packages })

    // Mock the behavior on addons install
    var addonsInstallTaskRun = td.function()
    td.when(addonsInstallTaskRun(td.matchers.anything())).thenDo(function (task) { addonsToInstall = task.packages })

    // Mock the behavior on packages uninstall
    var pkgsUninstallTaskRun = td.function()
    td.when(pkgsUninstallTaskRun(td.matchers.anything())).thenDo(function (task) { pkgsToUninstall = task.packages })

    // Mock the behavior on addon-install
    var taskFor = td.function()
    td.when(taskFor('npm-install')).thenReturn({ run: pkgsInstallTaskRun })
    td.when(taskFor('addon-install')).thenReturn({ run: addonsInstallTaskRun })
    td.when(taskFor('npm-uninstall')).thenReturn({ run: pkgsUninstallTaskRun })

    td.replace(Blueprint.prototype, 'taskFor', taskFor)

    pkgsToInstall = null
    addonsToInstall = null
    pkgsToUninstall = null
  })
